...
export default function CheckoutPage() {
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6 lg:px-8">
        ...
        <p className="mt-4 text-lg text-slate-600">
          Cảm ơn bạn đã tin tưởng. Mã đơn hàng của bạn là <span className="font-bold text-emerald-600">{lastOrder.id}</span>.
        </p>
        ...
        <div className="mt-12 rounded-2xl border border-slate-200 bg-white p-8 text-left shadow-sm">
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="border-t border-slate-100 pt-4 flex justify-between font-bold text-slate-900">
              ...
              <span className="text-xl text-emerald-600">{formatPrice(lastOrder.total)}</span>
            </div>
          </div>

          <div className="mt-8 grid gap-6 sm:grid-cols-2">
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Giao đến</h3>
              <p className="mt-1 text-sm font-medium text-slate-800">{lastOrder.customerInfo.name}</p>
              <p className="text-xs text-slate-500">{lastOrder.customerInfo.phone}</p>
              <p className="mt-1 text-xs text-slate-500">{lastOrder.customerInfo.address}, {lastOrder.customerInfo.city}</p>
            </div>
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Thanh toán</h3>
              <p className="mt-1 text-sm font-medium text-slate-800">
                {lastOrder.paymentMethod === 'cod' ? 'Thanh toán khi nhận hàng' : 
                 lastOrder.paymentMethod === 'bank' ? 'Chuyển khoản ngân hàng' : 'Ví MoMo'}
              </p>
            </div>
          </div>
        </div>
        ...
        {showToast && (
          <div className="fixed bottom-8 right-8 z-[100] flex animate-in fade-in slide-in-from-bottom-10 items-center gap-4 rounded-2xl border-2 border-emerald-500/20 bg-white/95 p-5 shadow-[0_20px_50px_rgba(16,185,129,0.3)] backdrop-blur-xl transition-all duration-500">
            ...
            <div>
              <p className="text-lg font-extrabold text-slate-900">Xác nhận đơn hàng!</p>
              <p className="text-sm font-medium text-slate-500">Hệ thống đang chuẩn bị giao cho bạn.</p>
            </div>
            <button
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <form onSubmit={handleSubmit} className="mt-8 gap-8 lg:grid lg:grid-cols-3">
          ...
          <div className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3 border-b border-slate-200 pb-4">
              {items.map(({ product, quantity }) => (
                <li key={product.id} className="flex items-start justify-between gap-x-8 text-sm">
